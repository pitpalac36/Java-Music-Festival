    ...
    public boolean findOne(String username, String password) {
        ...
        try (PreparedStatement statement = dbUtils.getConnection().prepareStatement("SELECT * FROM users WHERE username = ? AND password = ?")) {
            ...
            try (ResultSet result = statement.executeQuery()) {
                if (result.next()) {
                    logger.info(String.format("festival.model.domain.User with username : %s and password : %s was found!", username, password));
                    return true;
                } else {
                    logger.info(String.format("festival.model.domain.User with username : %s and password : %s not found!", username, password));
                    return false;
                }
            }
