    ...
    public Show findOne(int id) {
        ...
        try (PreparedStatement statement = dbUtils.getConnection().prepareStatement("SELECT * FROM shows WHERE id = ?")) {
            ...
            try (ResultSet result = statement.executeQuery()) {
                if (result.next()) {
                    logger.info(String.format("festival.domain.models.Show with id %s was found", id));
                    ...
                    logger.info(String.format("festival.domain.models.Show with id %s was not found", id));
                    return null;
                }
            }
