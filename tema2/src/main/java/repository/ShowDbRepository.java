...
public class ShowDbRepository implements IShowRepository {
    ...
    public List<ShowDTO> findArtists(String date) {
        ...
        List<ShowDTO> listResult = new ArrayList<>();
        try (PreparedStatement statement = dbUtils.getConnection().prepareStatement("SELECT * FROM shows WHERE dateOfEvent=?")) {
            logger.info(date);
            statement.setString(1, date);
            try (ResultSet result = statement.executeQuery()) {
                while (result.next()) {
                    ShowDTO current = null;
                    current = new ShowDTO(result.getString("artistName"), result.getString("location"), result.getString("dateOfEvent"), result.getInt("availableTicketsNo"));
