...
public class TestRepository {
    ...
    @Test
    public void testShowRepository() {
        loadProps();
        ...
        ShowDTO dto = new ShowDTO(show.getArtistName(), show.getLocation(), show.getDate(), show.getAvailableTicketsNumber());
        Assert.assertEquals(dto, repo.findArtists("2021-03-15" ).get(0));
        List<Show> shows = repo.findAll();
        Assert.assertTrue(shows.contains(show));
        String random = UUID.randomUUID().toString();
        repo.update(new Show(4, "artist" + random, String.valueOf(Date.from(Instant.now())), "location" + random, 100, 100));
